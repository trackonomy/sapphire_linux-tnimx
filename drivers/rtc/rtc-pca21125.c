...
static int pca21125_set_offset(struct device *dev, long offset)
{
	...
	return regmap_write(pca21125->map, pca21125_REG_OFFSET,
			    (unsigned int)reg);
}
...
static int pca21125_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	ret = regmap_update_bits(pca21125->map, pca21125_REG_CTRL2, CTRL2_AIE,
				 0);
	if (ret)
		return ret;

	/* Ensure alarm flag is clear */
	ret = regmap_update_bits(pca21125->map, pca21125_REG_CTRL2, CTRL2_AF,
				 0);
	if (ret)
		return ret;
...
static irqreturn_t pca21125_rtc_irq(int irq, void *dev)
{
	...
	if (val & CTRL2_AF) {
		...
		/* Clear alarm flag */
		regmap_update_bits(pca21125->map, pca21125_REG_CTRL2, CTRL2_AF,
				   0);
