...
/*
 * The datasheet calls for a minimum of 30ns pulse, and 600ns recovery time.
					      ...
					      uint16_t irq_mask)
{
	...
	ret = regmap_update_bits(pi4io->regmap, PI4IO16_INPUT_LATCH0, 0xFF,
				 irq_mask & 0xFF);
	if (ret) {
	...
	ret = regmap_update_bits(pi4io->regmap, PI4IO16_INPUT_LATCH1, 0xFF,
				 (irq_mask >> 8) & 0xFF);
	if (ret) {
					  ...
					  uint16_t irq_mask)
{
	...
	ret = regmap_update_bits(pi4io->regmap, PI4IO16_INTMASK_REG0, 0xFF,
				 ~(irq_mask & 0xFF));
	if (ret) {
	...
	ret = regmap_update_bits(pi4io->regmap, PI4IO16_INTMASK_REG1, 0xFF,
				 ~((irq_mask >> 8) & 0xFF));
	if (ret) {
...
static int pi4io16_gpio_setup(struct pi4io16_priv *pi4io)
{
	...
	if (of_property_read_string_array(dev->of_node, "gpio-line-names",
					  pi4io->gpio_names, gc->ngpio) < 0) {
	...
	if (ret) {
		dev_err(dev, "Failed to enable pull-ups/down on port0: %d",
			ret);
		return ret;
	}
	ret = regmap_write(pi4io->regmap, PI4IO16_PULLUP_ENB1, 0xFF);
	if (ret) {
		dev_err(dev, "Failed to enable pull-ups/down on port1: %d",
			ret);
		return ret;
	}
...
static irqreturn_t pi4io16_irq_handler(int irq, void *data)
{
	...
	while (pending) {
		...
		handle_nested_irq(
			irq_find_mapping(pi4io->gpio.irq.domain, level));
		/* clear pending bit */
		pending &= ~(1 << level);
	}
	return IRQ_HANDLED;
...
static int pi4io16_irq_setup(struct pi4io16_priv *pi4io)
{
	...
	if (irq < 0) {
		dev_err(dev, "No irq for gpio=%d rc=%d", desc_to_gpio(gpio),
			irq);
		return irq;
	}

	ret = devm_request_threaded_irq(dev, irq, NULL, pi4io16_irq_handler,
	...
	ret = gpiochip_irqchip_add_nested(&pi4io->gpio, &pi4io->irq_chip,
